...
// ⏱️ تنظیم زمان پری‌لودر (بر حسب میلی‌ثانیه - ۱ ثانیه = ۱۰۰۰)
...
export default function WorksPage() {
  ...
  useEffect(() => {
    const fetchProjects = async () => {
      try {
        // 🚀 استفاده از cache: 'no-store' برای گرفتن آخرین ادیت‌ها و اولویت‌ها بدون کش شدن
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      // 🚀 ظاهر شدن نرم کل صفحه بدون چشمک
      gsap.to(pageRef.current, { opacity: 1, duration: 1.2, ease: "power2.out" });

      // 🚀 انیمیشن ورود کلمه WORK (روی کلاس اعمال شده تا هم موبایل هم دسکتاپ کار کنه)
  ...
  const renderGridChunks = () => {
    const chunks = [];
    for (let i = 0; i < visibleCount && i < allProjects.length; i += 11) {
      chunks.push(allProjects.slice(i, i + 11));
    }
    ...
    return chunks.map((chunk, chunkIndex) => {
      ...
      return (
        <div key={chunkIndex} className="w-full flex flex-col items-center" style={{ marginTop: chunkIndex > 0 ? 'var(--row-gap)' : '0px' }}>
          ...
          {/* ردیف دوم: ۲ باکس بزرگ و مساوی (پروژه ۴ و ۵) که صفحه رو پر میکنن */}
          {(chunk[3] || chunk[4]) && (
          ...
          {/* ردیف سوم: ۳ باکس مساوی (پروژه‌های ۶، ۷، ۸) */}
          ...
          {/* ردیف چهارم: ۱ باکس بزرگ سمت راست، ۲ باکس کوچک سمت چپ (پروژه‌های ۹، ۱۰، ۱۱) */}
    ...
    <>
      ...
      <div ref={pageRef} className="page-wrapper relative w-full min-h-screen overflow-clip opacity-0" style={{ backgroundColor: bgColor }}>
        ...
        <style jsx global>{`
          ...
          .works-word-text {
            font-size: clamp(60px, 15vw, ${worksWordFontSize});
          }
          ...
          .burger-line-1.open {
            transform: translateY(var(--header-burger-trans)) rotate(45deg);
          }
          .burger-line-2.open {
            opacity: 0;
          }
          .burger-line-3.open {
            transform: translateY(calc(var(--header-burger-trans) * -1)) rotate(-45deg);
          }

          /* 🚀 ریسپانسیو شدن هوشمند فونت منو دقیقاً مثل صفحه جزئیات */
          @media (max-width: 768px) {
            :root {
              --header-menu-size: clamp(39px, 8.5vw, 67px) !important;
              --grid-gap: ${worksMobileGap};
              --row-gap: ${worksMobileGap};
            }

            .works-word-wrapper {
              left: ${worksMobilePadding} !important;
              top: 80px !important;
            }
        ...
        {/* 🚀 هدر مشکی و سالم با pointer-events-none */}
        <header className="fixed top-0 left-0 w-full z-50 h-[80px] md:h-[100px] bg-black pointer-events-none" dir="ltr">
          <div className="w-full relative h-full">
              ...
              <Link href="/" className="group relative flex items-center cursor-pointer" onClick={() => setMobileMenuOpen(false)}>
                <div className="flex items-center">
                  <div 
                    ...
                    style={{ color: cColor, width: "var(--header-logo-w)", height: "var(--header-logo-h)", '--hover-color': logoHoverColor } as React.CSSProperties}
                  >
                    <div 
                      className="absolute inset-0 border-current rounded-l-full border-y border-l"
                      style={{ borderRight: 'none', borderWidth: "var(--header-logo-thick)" }}
                    />
                  </div>
                  <span 
                    className={`transition-colors duration-500 group-hover:text-[var(--hover-color)] tracking-wider font-bold leading-none ${orbitronFont.className}`}
                    style={{ color: studioColor, fontSize: "var(--header-studio-size)", marginLeft: "var(--header-studio-overlap)", '--hover-color': logoHoverColor } as React.CSSProperties}
                  >
                    STUDIO
                  </span>
                </div>
            ...
            <div className="absolute z-[60] pointer-events-auto" style={{ right: "var(--header-burger-right)", top: "var(--header-burger-top)" }}>
              <button 
                className="group relative flex flex-col justify-between items-end cursor-pointer focus:outline-none" 
                style={{ width: "var(--header-burger-w)", height: "var(--header-burger-h)" }}
        ...
        {/* 🎯 منوی تمام‌صفحه دقیقاً کپی از فایل جزئیات */}
        <AnimatePresence>
          {mobileMenuOpen && (
            <motion.div 
            ...
            >
              <div className="relative w-full h-full max-w-[1920px] mx-auto pointer-events-auto flex flex-col md:block px-[30px] md:px-[100px] pt-[25vh] md:pt-[120px] pb-10">
                ...
                <motion.div 
                  ...
                  animate="animate" 
                  exit="initial" 
                  className="flex-1 flex flex-col justify-start items-start md:absolute md:right-[100px] md:bottom-[51px] md:h-auto" 
                  dir="rtl"
                >
                  {navLinks.map((link) => (
                ...
                >
                  ...
                  <a 
                    ...
                    className="text-zinc-400 font-light leading-relaxed text-sm md:text-lg mb-6 md:mb-8 hover:text-white transition-colors text-right md:text-left" 
        ...
        {/* =====================================================================
            بخش اصلی محتوای نمونه کارها (WORKS)
        ===================================================================== */}
        <main 
        ...
        >
          <div 
            className="works-word-wrapper hidden md:block"
            dir="ltr"
          >
            <span 
              className="block uppercase tracking-tight leading-none drop-shadow-2xl works-word-text"
              style={{ color: textColor, fontFamily: englishFontFamily, fontWeight: 900 }}
            >
              {worksWordText}
            </span>
          </div>
          ...
          <div 
            className="works-word-wrapper md:hidden"
            dir="ltr"
          >
            <span 
              className="block uppercase tracking-tight leading-none drop-shadow-2xl works-word-text"
          ...
          <div 
            className="grid-main-container w-full relative z-20 flex flex-col items-center" 
            style={{ transform: `scale(${worksBoxGlobalScale})`, transformOrigin: 'top center' }}
          >
            {isLoading ? (
              <div className="w-12 h-12 mt-32 border-2 border-zinc-800 border-t-white rounded-full animate-spin opacity-0" />
            ) : allProjects.length === 0 ? (
              <p className="text-zinc-500 mt-32 text-xl opacity-0">هیچ پروژه‌ای در دیتابیس یافت نشد.</p>
            ) : (
              <>
                {renderGridChunks()}
                
                {!isPreloading && visibleCount < allProjects.length && (
